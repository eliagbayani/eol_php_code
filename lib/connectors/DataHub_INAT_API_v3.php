...
class DataHub_INAT_API_v3
{
    ...
    function get_iNat_taxa_observation_using_API($group, $grade, $include_rank) //not advisable to use, bec. of the 10,000 limit page coverage
    {
        ...
        else $main_url = str_replace("XRANK", $include_rank, $this->inat_api['family_genus']); //for 'family' and 'genus'
    ...
    private function gen_iNat_info_taxa_using_DwCA()
    {
        ...
        $options['expire_seconds'] = 60*60*24*30*1; //3 months cache
        ...
        /* development only
        $paths = Array(
            'archive_path' => '/Volumes/AKiTiO4/eol_php_code_tmp/dir_64322/',
            'temp_dir'     => '/Volumes/AKiTiO4/eol_php_code_tmp/dir_64322/'
        );
        */
    ...
    {   echo "\nReading file, task: [$what]...[$quality_grade]\n";
        ...
        foreach(new FileIterator($file) as $line => $row) { $i++; // $row = Functions::conv_to_utf8($row);
            ...
            if($i == 1) $fields = explode("\t", $row);
                ...
                // elseif($what == 'assemble data from 3 TSVs' && $quality_grade) {} //obsolete
            }
        }
    }
