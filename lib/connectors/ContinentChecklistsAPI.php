...
class ContinentChecklistsAPI
{
    ...
    {   /* copied template
        ...
        */
        ...
        $this->debug = array();
        $this->bibliographicCitation = "GBIF.org (23 January 2025) GBIF Occurrence Download https://doi.org/10.15468/dl.e23myp"; //with filtered datasetKeys
            // "GBIF.org (9 January 2025) GBIF Occurrence Download https://doi.org/10.15468/dl.pr4z3z"; //todo: get it dynamically
        ...
        $this->report_1 = $this->destination . "continents.tsv";
        $this->report_2 = $this->destination . "run_continents.tsv";

        if(!is_dir($this->destination)) mkdir($this->destination);
        $this->continent_path = $this->destination.'continents';
        if(!is_dir($this->continent_path)) mkdir($this->continent_path);
    ...
    function generate_report($what) //'waterbodies' or 'countries' or 'continents'
    {
        ...
        if($what == 'countries') $report = str_replace('Continent_checklists', 'Country_checklists', $report);
        if(file_exists($report)) unlink($report);
        // /*
        require_library('connectors/DwCA_Utility');
        // */
        $file = $this->destination . "$what.tsv";
        if($what == 'countries') $file = str_replace('Continent_checklists', 'Country_checklists', $file);
    ...
    function start($fields) //start($counter = false, $task, $sought_continent = false) //$counter is only for caching
    {   //exit("\n[$counter]\n");
        /* may not need this anymore...
        require_library('connectors/GBIFdownloadRequestAPI');
        $func = new GBIFdownloadRequestAPI('Continent_checklists');
        $key = $func->retrieve_key_for_taxon('Continent_checklists');
        echo "\nkey is: [$key]\n";
        */
        ...
        elseif($task == 'show_continents_metadata') {
            self::show_continents_metadata();
            return;
        }
        ...
        if($task == 'divide_into_continent_files') {
            // /* remove current /continents/ folder
            recursive_rmdir($this->continent_path); echo ("\nFolder removed: " . $this->continent_path);
            if(!is_dir($this->continent_path)) mkdir($this->continent_path);
            // */
    ...
    private function compare_continents()
    {
        ...
        echo "\nTotal AnneT continents: [".count($this->AnneT_continents)."]";
        $diff = count($this->AnneT_continents) - count($not_found);
    ...
    {   $cont = false; //debug only
        ...
        self::initialize();
        $files = $this->continent_path . "/*.tsv"; echo "\n[$files]\n"; $i = 0;
        // foreach(glob($files) as $file) { //echo "\n$file\n"; exit;
        foreach(new FileIterator($this->destination.'continents_AnneThessen.tsv') as $line => $row) { $i++; //e.g. "Adriatic Sea"
            if(!trim($row)) continue;
    ...
    private function evaluate_continent_file($file)
    {   
        exit("\nnot applicable 1\n");
        /* not applicable
        $ret = self::get_continent_name_from_file($file); //e.g. $file "/Volumes/Crucial_4TB/other_files/GBIF_occurrence/Continent_checklists/continents/AD.tsv"
        $continent_name_lower = $ret['lower_case'];
        $this->continent_name = $ret['orig'];
        print_r($ret); exit;
        if(!in_array($this->continent_name, $this->AnneT_continents)) {
            if($val = @$this->continent_map[$this->continent_name]) {
                $this->continent_name = $val;
                if(!in_array($this->continent_name, $this->AnneT_continents)) {
                    echo "\nNot mapped* [$this->continent_name]";
                    $this->debug['Not mapped*'][$this->continent_name] = '';
    ...
    private function create_individual_continent_checklist_resource($counter = false, $task, $sought_continent = false)
    {
        ...
        // foreach(new FileIterator($this->destination.'continents_AnneThessen.tsv') as $line => $row) { $i++;
        //     if(!$row) continue;
        //     echo "\n$row";
        // }//foreach()
        // exit("\nend 2\n");
        ...
        foreach(new FileIterator($this->destination.'continents_AnneThessen.tsv') as $line => $row) { $i++; //e.g. "Adriatic Sea"
            ...
            */
            ...
            $continent_name_lower = str_replace(" ", "_", strtolower($row)); //e.g. "adriatic_sea"
            $file = $this->continent_path . "/".$continent_name_lower.".tsv";
            ...
            if($this->continent_name = $row) {
                // $continent_name_lower = $ret['lower_case'];
                // $this->continent_name = $ret['orig'];

                /* manual filter, dev only
                if(in_array($this->continent_name, array('Philippines'))) continue;
                */

                if($sought_continent) {
                    if(!in_array($this->continent_name, array($sought_continent))) continue;
                }
            ...
            // if(substr($continent_name_lower,0,4) == "the ")                                               $continent_name_lower = str_ireplace("the ", "", $continent_name_lower); //The Bahamas => SC_bahamas.tar.gz
                if(strtolower($this->continent_name) == strtolower("Democratic Republic of the Congo"))   $continent_name_lower = "congo";
            elseif(strtolower($this->continent_name) == strtolower("Republic of the Congo"))              $continent_name_lower = "repubcongo";

            // $folder = "SC_".$continent_name_lower; //obsolete
            ...
            self::proc_continent($folder, $file);
            // break; //debug only | process just 1 country
            // if($i >= 3) break; //dev only
        }
    }
    private function proc_continent($folder, $file)
    {
    ...
    private function process_continent_file($rec)
    {   /*Array(
            [specieskey] => 1710962
            [SampleSize] => 16
            [countrycode] => AD
    ...
    private function save_to_different_continent_files($rec)
    {   /*Array(
            [specieskey] => 1000607
            [COUNT(specieskey)] => 2
        ...
        foreach($continents as $continent) {
            ...
            $continent = self::massage_continent($continent);
            @$this->save[$continent][$specieskey] += $count;
        }
    }
    private function write_continent_tsv_files()
    {
        ...
        foreach($this->AnneT_continents as $continent) {
            ...
            foreach($this->save[$continent] as $specieskey => $count) { // echo "\n[$continent] [$specieskey] [$count]";
                // /* start writing OK
    ...
    private function download_extract_gbif_zip_file()
    {
        ...
        /* during dev only
        return "/Volumes/AKiTiO4/other_files/GBIF_occurrence/Continent_checklists/0036064-241126133413365.csv";
        */
    }
    /* not applicable
    private function initialize_continents_from_csv()
    {
        $final = array();
        $options = $this->download_options;
        $options['expire_seconds'] = false;
    ...
    private function get_continent_name_from_file($file) //e.g. $file "/Volumes/Crucial_4TB/other_files/GBIF_occurrence/Continent_checklists/continents/AD.tsv"
    {
        $abbrev = pathinfo($file, PATHINFO_FILENAME); //e.g. "PH"
        if($continent_name = @$this->country_code_name_info[$abbrev]) {
            $lower = strtolower(str_replace(" ", "", $continent_name));
            echo "\nCountry: [$abbrev] [$continent_name] [$lower]\n";
            return array('lower_case' => $lower, 'orig' => $continent_name, 'abbrev' => $abbrev);
        }
    ...
    private function get_continent_uri($continent, $what = 173)
    {   //Antigua and Barbuda; what is saved in EOL terms file is: "Antigua And Barbuda"
        $continent = str_replace(" and ", " And ", $continent);
        $continent = str_replace(" of ", " Of ", $continent);
        ...
        if($uris = @$this->value_uris[$continent]) {
            ...
            */
            // /*
            switch ($continent) { //put here customized mapping
        ...
        if(substr($continent, 0, 4) == 'The ') {
            $continent = trim(substr($continent, 3, strlen($continent)));
            // echo "\n----------------------------try again ($continent)\n";
            if($uri = self::get_continent_uri($continent, 2)) return $uri;
        }
        // */


        // print_r($this->values_uri); //debug only
        echo ("\nNo URI for [$what] [$continent]"); //print_r($this->value_uris); print_r($this->value_uris[$continent]);  exit("\nstop munax\n");
        $this->debug['No URI for country'][$continent][$what] = '';
        return false;
    }
